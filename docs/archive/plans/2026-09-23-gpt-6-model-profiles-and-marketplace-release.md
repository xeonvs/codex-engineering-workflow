...
| REQ-002 | All active source/package version owners and generated bytes agree in a new source release. | User request; repository release contract | WQ-02 | Full release/security gate, package validators, PR merge, annotated tag and release readback | done |
| REQ-003 | xeonvs-engineering imports exact immutable source bytes and publishes its own release while preserving tgrep-search. | User request; marketplace sync contract | WQ-03 | Recorded provenance/byte verification, catalog tests, release workflow and asset readback | done |
| REQ-004 | Local Codex and Claude managed marketplaces and workflow installations resolve the published version. | Prior user preference; current release request | WQ-04 | Native CLI update and active version readback | done |
| REQ-005 | Source and marketplace plans close with durable release evidence. | Workflow lifecycle contract | WQ-05 | Lifecycle check and closure readback | done |
...
- [x] WQ-02 — Bump source version, validate, review, merge, tag and release for REQ-002. `done`
- [x] WQ-03 — Import, validate, review, merge and release xeonvs-engineering for REQ-003. `done`
- [x] WQ-04 — Refresh and read back local managed installations for REQ-004. `done`
- [x] WQ-05 — Reconcile and close source and marketplace plans for REQ-005. `done`
...
- 2026-09-23: Final source release gate passed 12/12 on reviewed bytes: 266 tests (one skipped), privacy and Gitleaks checks green. PR #15 merged at `9a28224af9efc329ed420ddcbd25b1f0aa354565`; annotated `v0.9.8` and GitHub release published.
- 2026-09-23: Marketplace PR #19 merged at `1a501c87262d9f91dd4953526f34c21bf99b83a8`; annotated `v1.0.8`, release CI, archive and SHA256SUMS passed. Recorded-byte verification preserved tgrep-search 1.0.3. Codex marketplace upgrade and Claude marketplace/plugin update read back engineering-workflow 0.9.8 enabled in both, with tgrep-search 1.0.3 enabled.
...
- None; all delivery work is complete. Claude reports that a restart is needed to activate updated plugin bytes in already-running sessions.
...
- [x] Final source, marketplace, release, installation, and plan states agree.

### Closure Gate

- [x] All requirements and queue items are terminal with applicable validation and delivery evidence.

### Post-Close Delivery

- Source and marketplace publication plus local refresh completed before closure.
